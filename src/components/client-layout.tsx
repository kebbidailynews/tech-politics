...
import { Menu, X, Search, TrendingUp, Clock, DollarSign, Download } from 'lucide-react';
        ...
        <>
          ...
          <aside className="fixed left-0 top-0 h-full w-80 bg-white dark:bg-neutral-900 shadow-2xl z-50 overflow-y-auto">
            ...
            <nav className="p-5 space-y-6">
              <div>
                <h3 className="font-bold text-sm uppercase text-gray-600 dark:text-gray-400 mb-3">Pages</h3>
                {[
                  ...
                  <Link key={i.href} href={i.href} className="block py-2 text-lg font-medium hover:text-red-600" onClick={() => setSidebarOpen(false)}>
                    {i.label}
                  </Link>
                ))}
              </div>
              ...
              <div>
                ...
                {categories.map(cat => (
                  <Link key={cat._id} href={cat.slug} className="block py-2 text-lg font-medium hover:text-red-600" onClick={() => setSidebarOpen(false)}>
                    {cat.title}
                  </Link>
                ))}
              </div>
